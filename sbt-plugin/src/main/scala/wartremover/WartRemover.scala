...
object WartRemover extends sbt.AutoPlugin {
  ...
  def wartremoverRunTask(c: Configuration): Def.Initialize[InputTask[Option[CompileAnalysis]]] = Def.inputTask {
    ...
    val sourceFiles = {
      ...
      (token(Space) ~> token(fileParser(file("/")) || basicUri, "wartremover source file or url")).+ <~ SpaceClass.*
    }.parsed.flatMap {
      case Left(arg) =>
        getSources(arg)
      case Right(arg) =>
    ...
    IO.withTemporaryDirectory { dir =>
      ...
      if (classes.isEmpty) {
        ...
        if (impl.isEmpty) {
          ...
          val s = (thisProjectRef / state).value.appendWithSession(
            Def.settings(
