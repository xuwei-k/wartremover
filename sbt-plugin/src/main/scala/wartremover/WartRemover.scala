...
object WartRemover extends sbt.AutoPlugin {
  ...
  object autoImport {
    ...
    val wartremoverRun = inputKey[Option[CompileAnalysis]]("run input wartremover by task")
  ...
  private[this] def getJar(files: Set[String]): Def.Initialize[Task[Seq[Byte]]] = Def.task {
    val key = WartCacheKey(
      scalaV = (wartremoverRun / scalaVersion).value,
      files = files
    )
    val forkOps = (wartremoverRun / forkOptions).value
  ...
  def wartremoverRunTask(c: Configuration): Def.Initialize[InputTask[Option[CompileAnalysis]]] =
    Def.inputTaskDyn {
          ...
          if (classes.isEmpty) {
            ...
            val impl = classes.filter { className =>
              ...
              if (scalaBinaryVersion.value == "3") {
                ...
                clazz.getInterfaces.exists(_.getName == traverserName)
              }
            }.map { s =>
            ...
            if (impl.isEmpty) {
              ...
              val oldOptions = (c / compile / scalacOptions).value
              Def.taskDyn {
                ...
                Def.taskDyn {
                  Project.runTask(thisProjectRef.value / c / compile / scalacOptions, state.value)
                  Def.taskDyn {
                    val result = (c / compile).value
                    log.info(s"compiled $result")
                    Def.taskDyn {
                      log.info(s"revert ${(c / wartremoverOptionsFile).value}")
                      IO.writeLines(
                        (c / wartremoverOptionsFile).value,
                        oldOptions,
                      )
  ...
  override lazy val projectSettings: Seq[Def.Setting[_]] = Def.settings(
    ...
    Compile / compile / scalacOptions := {
      ...
      val log = streams.value.log
      val default = (Compile / compile / scalacOptions).value
      if (f.isFile) {
    ...
    Test / compile / scalacOptions ++= {
      ...
      val log = streams.value.log
      val default = (Test / compile / scalacOptions).value
      if (f.isFile) {
        val lines = IO.readLines(f)
        log.info(s"options = $lines")
        lines
